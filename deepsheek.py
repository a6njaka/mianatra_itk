...
class MediaPlayer:
    ...
    def play_media(self, file_path):
        # Stop any existing media thread
        if self.media_thread and self.media_thread.is_alive():
            self.player.stop()
            self.is_playing = False
            self.media_thread.join(timeout=1.0)

        # Start new media thread
        self.media_thread = threading.Thread(target=self._play_media_thread, args=(file_path,))
        self.media_thread.daemon = True
        self.media_thread.start()

    def _play_media_thread(self, file_path):
        ...
        # Get the handle of the panel - must be done in main thread
        wx.CallAfter(self._set_media_handle)
        ...
        wx.CallAfter(setattr, self.parent_frame, 'playing_video', True)

    def _set_media_handle(self):
    ...
    def on_media_end(self, event):
        if self.is_playing:  # Check if still playing to avoid multiple calls
            self.is_playing = False
            wx.CallAfter(self.panel.Hide)  # Hide the panel when video ends
            ...
            if re.search("course.*\\.(mp4|avi)$", self.file_path, re.I):
                ...
                wx.CallAfter(setattr, self.parent_frame, 'playing_video', False)


class ExoLoaderThread(threading.Thread):
    ...
    def run(self):
        try:
            wx.CallAfter(self.frame.SetStatusText, "Loading exercises...", 0)
            p1 = load_exo.ExoSchedule()
            self.frame.all_exo = p1.all_exo
            self.frame.exo_list = list(self.frame.all_exo)
            self.frame.all_exo_completed = False

            wx.CallAfter(self.frame.verify_correctness_all_exo)
            wx.CallAfter(self.callback)
            wx.CallAfter(self.frame.SetStatusText, "Exercises loaded", 0)
        except Exception as e:
            wx.CallAfter(self.frame.SetStatusText, f"Error loading exercises: {e}", 0)
            print(f"Error in ExoLoaderThread: {e}")
...
class MyFrame(wx.Frame):
    ...
    def change_bitmap_buttons(self, new_img, max_height=150):
        ...
        # Load images in a thread to avoid UI freezing
        thread = threading.Thread(target=self._change_bitmap_buttons_thread, args=(new_img, max_height))
        thread.daemon = True
        thread.start()

    def _change_bitmap_buttons_thread(self, new_img, max_height=150):
        try:
            for index, img_data in enumerate(new_img):
                if index >= len(self.bitmap_buttons):
                    break
                ...
                try:
                    ...
                    wx.CallAfter(self.bitmap_buttons[index].SetBitmapLabel, bitmap)
                    wx.CallAfter(self.bitmap_buttons[index].SetMinSize, bitmap.GetSize())
                    wx.CallAfter(self.bitmap_buttons[index].Show)
                except Exception as e:
                    print(f"Error loading image: {e}")
        except Exception as e:
            print(f"Error in change_bitmap_buttons_thread: {e}")
    ...
    def load_image(self, img_path):
        # Load image in a thread to avoid UI freezing
        thread = threading.Thread(target=self._load_image_thread, args=(img_path,))
        thread.daemon = True
        thread.start()

    def _load_image_thread(self, img_path):
        try:
            if isinstance(img_path, str) and os.path.isfile(img_path):
                ...
                wx.CallAfter(self.background_staticbitmap.SetBitmap, new_bitmap)
                wx.CallAfter(self.home_panel.Refresh)
            ...
            # Bytes_Type
                ...
                wx.CallAfter(self.background_staticbitmap.SetBitmap, wx_bitmap)
                wx.CallAfter(self.home_panel.Refresh)
            ...
            print(f"Error loading image in thread: {e}")

        wx.CallAfter(self.home_panel.Layout)

    def load_all_exo(self):
        print("---->>load_all_exo")
        # Use thread to load exercises
        loader_thread = ExoLoaderThread(self, self._after_exo_loaded)
        loader_thread.start()

    def _after_exo_loaded(self):
        self.stage_index_done = []
        self.get_exo_name()
...
class Setting_DLG(wx.Dialog):
    ...
    def update_list_exo(self):
        ...
        # Use thread to load exercise list to avoid UI freezing
        thread = threading.Thread(target=self._update_list_exo_thread, args=(all_exo_path,))
        thread.daemon = True
        thread.start()

    def _update_list_exo_thread(self, all_exo_path):
        exo_list = []
        ...
        # Update UI in main thread
        wx.CallAfter(self._update_list_exo_ui, exo_list)

    def _update_list_exo_ui(self, exo_list):
        self.list_all_exo = exo_list
        for exo in exo_list:
